/*
 *所有关于Viat_wk_ord_detail类的业务代码应在此处编写
*可使用repository.调用常用方法，获取EF/Dapper等信息
*如果需要事务请使用repository.DbContextBeginTransaction
*也可使用DBServerProvider.手动获取数据库相关信息
*用户信息、权限、角色等使用UserContext.Current操作
*Viat_wk_ord_detailService对增、删、改查、导入、导出、审核业务代码扩展参照ServiceFunFilter
*/
...
{
    public partial class Viat_wk_ord_detailService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IViat_wk_ord_detailRepository _repository;//访问数据库

        [ActivatorUtilitiesConstructor]
        public Viat_wk_ord_detailService(
            IViat_wk_ord_detailRepository dbRepository,
        ...
        /// <returns></returns>
        public List<Viat_wk_ord_detail> getDataByBidMasterDBID(string bidmast_dbid)
        {
            string sSql = "select * from viat_wk_ord_detail  where bidmast_dbid='" + bidmast_dbid + "'";

            return _repository.DapperContext.QueryList<Viat_wk_ord_detail>(sSql, null);
        }
