...
namespace VIAT.WorkFlow.Services
{
    public partial class View_cust_order_transferService
    {
        ...
        public PageGridData<View_cust_order_transfer> GetPageDataByOrderNO(PageDataOptions pageData)
        {
           ...
           left join (SELECT top 1 prod_dbid,cust_dbid,min_qty
								from viat_app_cust_price_detail WHERE status='Y' AND  SysDateTime() >= start_date ORDER by created_date desc
								union 
								SELECT top 1  g.prod_dbid,g.cust_dbid,p.min_qty from viat_app_cust_group g left join viat_app_cust_price p
								on g.pricegroup_dbid=p.pricegroup_dbid and p.prod_dbid=g.prod_dbid 
								WHERE p.status='Y' AND  g.status='Y' and SysDateTime() >= p.start_date order by g.created_date desc)
        ...
        public PageGridData<View_cust_order_transfer> GetAllPageData(PageDataOptions pageData)
        {
