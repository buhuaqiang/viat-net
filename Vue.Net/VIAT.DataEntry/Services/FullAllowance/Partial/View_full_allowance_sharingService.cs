/*
 *所有关于View_full_allowance_sharing类的业务代码应在此处编写
*可使用repository.调用常用方法，获取EF/Dapper等信息
*如果需要事务请使用repository.DbContextBeginTransaction
*也可使用DBServerProvider.手动获取数据库相关信息
*用户信息、权限、角色等使用UserContext.Current操作
*View_full_allowance_sharingService对增、删、改查、导入、导出、审核业务代码扩展参照ServiceFunFilter
*/
...
{
    public partial class View_full_allowance_sharingService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IView_full_allowance_sharingRepository _repository;//访问数据库

        [ActivatorUtilitiesConstructor]
        public View_full_allowance_sharingService(
            IView_full_allowance_sharingRepository dbRepository,
        ...
        public override PageGridData<View_full_allowance_sharing> GetPageData(PageDataOptions options)
        {
            PageGridData<View_full_allowance_sharing> pageGridData = new PageGridData<View_full_allowance_sharing>();
            /*解析查询条件*/
            ...
            QuerySql = "select ROW_NUMBER()over(order by tab4.cust_id desc) as rowId, tab4.* from( " +
            ...
            pageGridData.rows = repository.DapperContext.QueryList<View_full_allowance_sharing>(sql, null);
