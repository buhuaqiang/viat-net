    ...
    public partial class View_cust_price_detailService
    {
        ...
        public override PageGridData<View_cust_price_detail> GetPageData(PageDataOptions options)
        {
            ...
            Dictionary<string, string> groupAlias = new Dictionary<string, string>() {
                { "cust_id", "cust" },{ "start_date","custGroup"} ,{ "end_date","custGroup"},{ "updated_date","custGroup"},
                 { "prod_id", "prod" },{ "cust_dbid","custGroup"} ,{ "state","prod"}
            };
...
　　　                            AND prod_dbid =  '" + entityParameter.prod_dbid + @"' 
        ...
        private void AddCustPriceData(Viat_app_cust_price_detail entity, SaveModel saveModel)
        {
            ...
            DateTimeFormatInfo dtFormat = new DateTimeFormatInfo();
            dtFormat.ShortDatePattern = "yyyy-MM-dd";
            if (getFormatYYYYMMDD(entity.end_date) < getFormatYYYYMMDD(System.DateTime.Now))
            {
                entity.status = "N";
            }
            else
        ...
        {
            //如果下載模板指定了DownLoadTemplate,則在Import方法必須也要指定,並且字段要和下載模板裡指定的一致//x.cust_id, x.prod_id, x.nhi_price,x.invoice_price, x.net_price,x.reserv_price, x.gross_price, x.min_qty, x.start_date, x.end_date, x.remarks
            DownLoadTemplateColumns = x => new {
                x.cust_id,
                x.cust_name,
                x.prod_id,
