/*
 *所有关于Viat_app_power_contract_free_prod类的业务代码应在此处编写
...
{
    public partial class Viat_app_power_contract_free_prodService
    {
        private readonly IHttpContextAccessor _httpContextAccessor;
        private readonly IViat_app_power_contract_free_prodRepository _repository;//访问数据库

        [ActivatorUtilitiesConstructor]
        public Viat_app_power_contract_free_prodService(
            IViat_app_power_contract_free_prodRepository dbRepository,
            ...
            if (!string.IsNullOrEmpty(options.Wheres))
            {
                ...
                if (searchParametersList != null && searchParametersList.Count > 0)
                {
                    ...
                    QuerySql = @"SELECT
	                            ...
	                            comprod.prod_ename
                                FROM
                                Viat_app_power_contract_free_prod free
                                LEFT JOIN Viat_com_prod comprod ON free.prod_dbid = comprod.prod_dbid " +
                             " where free.powercont_dbid='" + sPowerContDbid + "'";
                }
            }
